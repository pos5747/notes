...
Probability theory begins with the idea of a **random process**, which is an "experiment" or procedure whose outcome is not known in advance.
...
The probability function $\Pr(\cdot)$ must satisfy the following three rules.

::: {#thm-axioms}
Let $P$ be a probability function defined on a sample space $\Omega$. Then:

1. $\Pr(A) \ge 0$ for all events $A$.
2. $\Pr(\Omega) = 1$.
3. For every infinite sequence of disjoint events $A_1, A_2, ...$, $\Pr \left( \displaystyle \bigcup_{i = 1}^\infty A_i \right) = \displaystyle \sum_{i = 1}^\infty \Pr(A_i)$.^[Some notes on Axiom 3. Examples of an infinite sequence of disjoint events? For $\Omega = \mathbb{R}^+$? For $\Omega =\{0, 1\}$? An infinite sequence of disjoint events is difficult to conceptualize. For $\Omega = \mathbb{R}^+$, *one* such sequence would be $[0, 1), [1, 2), [2, 3), ...$. For $\Omega =\{0, 1\}$, *one* such sequence would be $\{0\}, \{1\}, \emptyset, \emptyset, \emptyset,...$.]
:::
...
1. $\Pr(\{\text{H}\}) = 0.5 \ge 0$, $\Pr(\{\text{T}\}) = 0.5 \ge 0$
1. $\Pr(\Omega) = \Pr(\{\text{H}, \text{T}\}) = 0.5 + 0.5 = 1$
1. Let $A = \{\text{H}\}$, $B = \{\text{T}\}$. These are disjoint, so, $\Pr(A \cup B) = \Pr(A) + \Pr(B) = 0.5 + 0.5 = 1$.
:::
...
::: {#def-partition}
To create a **partition** $B_1, B_2, ..., B_k$ of the sample space $\Omega$, divide $\Omega$ into $k$ disjoint events $B_1, B_2, ..., B_k$ so that $\bigcup_{i = 1}^k B_i = \Omega$.
:::

::: {#thm-ltp}
**Law of Total Probability**  
Suppose a partition $B_1, B_2, ..., B_k$ of the sample space $\Omega$ where $\Pr(B_j) > 0$ for $j = 1, 2, ... , k$. Then
...
::: {#thm-bayes}
**Bayes' Rule**  
Suppose a partition $B_1, B_2, ..., B_k$ of the sample space $\Omega$ where $\Pr(B_j) > 0$ for $j = 1, 2, ... , k$. Suppose an event $A$, where $\Pr(A) > 0$. Then
...
Suppose the simple partition $B$ and $B^c$ of the sample space $\Omega$ where $\Pr(B) > 0$ and $\Pr(B^c) > 0$. Suppose an event $A$, where $\Pr(A) > 0$. Then
...
The **CDF** $F(x)$ of a random variable $X$ is $F(x) = \Pr(X \le x)$.
...
- Continuous: $f(x, y)$ such that $\Pr((X, Y) \in A) = \iint_A f(x, y)\, dx\, dy$
...
- Discrete: $\Pr(Y = y \mid X = x) = \frac{\Pr(X = x, Y = y)}{\Pr(X = x)}$
...
Compute $\Pr(X = 1)$, $\Pr(Y = 0)$, and $\Pr(Y = 1 \mid X = 1)$.

*Solution.*

- $\Pr(X = 1) = 0.2 + 0.4 = 0.6$
- $\Pr(Y = 0) = 0.1 + 0.2 = 0.3$
- $\Pr(Y = 1 \mid X = 1) = \frac{0.4}{0.6} = 0.\overline{6}$
...
- $\mathbb{E}[XY] = (0)(0) \cdot 0.1 + (0)(1) \cdot 0.3 + (1)(0) \cdot 0.2 + (1)(1) \cdot 0.4 = 0.4$.

Only the cell with $X = 1$ *and* $Y = 1$ contributes to $\mathbb{E}[XY]$, because $XY = 0$ whenever either variable is 0.

Then $\text{Cov}(X, Y) = 0.4 - (0.6)(0.7) = 0.4 - 0.42 = -0.02$.
