...
Sometimes the interval has no upper bound. We handle an unbounded interval by integrating to a finite endpoint $b$ and then letting $b$ grow without bound.

::: {#def-improper}
The **improper integral** of $f(x)$ from $a$ to $\infty$ is

$$
\int_a^\infty f(x)\, dx = \lim_{b \to \infty} \int_a^b f(x)\, dx
$$

If this limit exists and is finite, the integral **converges**. Otherwise, the integral **diverges**.
:::

::: {#exm-improper1}
Compute $\int_0^\infty e^{-x}\, dx$.
:::

*Solution.* Find the antiderivative $F(x) = -e^{-x}$ and evaluate it from $0$ to a finite endpoint $b$, so that

$$
\int_0^b e^{-x}\, dx = -e^{-b} - \left( -e^{0} \right) = 1 - e^{-b}
$$

Now let $b$ grow without bound. Since $e^{-b} \to 0$ as $b \to \infty$, we obtain

$$
\int_0^\infty e^{-x}\, dx = \lim_{b \to \infty} \left( 1 - e^{-b} \right) = 1
$$

Improper integrals come up whenever we work with a continuous random variable defined on $[0, \infty)$, because its probability density function accumulates to one over that whole range. The exponential distribution is the case we use most.

When you integrate by parts on an unbounded interval, the boundary term $\left[ u v \right]_0^\infty$ is also a limit. Terms like $b e^{-\lambda b}$ and $b^2 e^{-\lambda b}$ go to zero as $b \to \infty$, because $e^{-\lambda b}$ shrinks faster than any power of $b$ grows.
...
This matches the exact value $\frac{7}{3} \approx 2.3333$.
...
This matches the exact value $-\frac{1}{4}$.
