...
An intercept (or column of ones) is added to the design matrix by default. You can remove the intercept by adding `-1` or `+ 0` to the formula. 
...
A formula with `species` on the right-hand side makes a design matrix with an intercept and two indicators. One indicator variable is for Chinstrap penguins; the other is for Gentoo penguins. Adelie penguins are the reference (i.e., left-out) category. 
...
The resulting design matrix includes columns for the intercept, the dummy variables for `species`, and the continuous predictor. The coefficients on the indicator variables represent differences between species, but now "holding flipper length constant."
...
Sometimes we suspect the effect of a variable on the outcome is not linear. To allow nonlinear effects, we can include polynomial terms of the predictor in the design matrix. For example, we can use the quadratic model
...
With orthogonal polynomials, the columns are not $x$ and $x^2$ but two uncorrelated combinations of them. With raw polynomials, the columns are $x$ and $x^2$ exactly. 
...
If we want the arithmetic version instead of the formula-operator version, we must wrap the expression in `I()`.
...
Sometimes we want to adjust an existing formula instead of rewriting it from scratch. The function `update()` makes this easy. It is especially useful when experimenting with *changes* to a baseline specification.
