...
## Summary of four common models

Just as an example of the kinds of models we can develop using this approach, here are four common models. 

1. Normal linear: common for unbounded outcomes (without severe outliers)
2. Logit: common for binary outcomes
3: Negative binomial: common four count data (allows for overdispersion, unlike the Poisson)
4: Weibull: common for time-to-event or duration data, though we have many options here and Weibull might not be the best.
...
| Function     | `lm()` or `glm(..., family = gaussian)`                  |
...
|------------------|-----------------------------------------------------------------------------|
| Outcome          | Binary                                                                      |
| Model            | $y_i \sim \text{Bernoulli}(\pi_i)$, where $\pi_i = \operatorname{logit}^{-1}(X_i \beta)$   |
| Expected value   | $\hat{\mu} = \hat{\pi} = \operatorname{logit}^{-1}(X_c \hat{\beta})$  |
| First difference | $\hat{\Delta} = \hat{\pi}_{hi} - \hat{\pi}_{lo} = \operatorname{logit}^{-1}(X_{hi} \hat{\beta}) - \operatorname{logit}^{-1}(X_{lo} \hat{\beta})$ |
