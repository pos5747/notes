...
The normal model cannot describe a binary outcome well, because it doesn't make much conceptual sense to model 0s and 1s as following a normal distribution. That said, we can use the linear model (i.e., OLS) with binary outcome variables.

-   Recall that the linear model is represented by the equation $E(y_i) = X_i\beta$.
...
-   Since $y_i$ is binary, $E(y_i) = \Pr(y_i = 1) = \Pr(y_i)$, giving us $\Pr(y_i) = X_i\beta$.

The LPM has two advantages:

1.  It's *very* easy to estimate (i.e., OLS; $\hat{\beta} = (X'X)^{-1}X'y$).
2.  It is easy to interpret (i.e., a one unit change in $x_j$ leads to a $\hat{\beta}_j$ unit increase in $\Pr(y)$).
...
2.  **Conditional Heteroskedasticity** The normal-linear model assumes a constant variance $\sigma^2$. However, it is impossible to have homoskedastic residuals of a binary outcome if the probability of an event varies. Specifically, if $y_i$ is binary, then $\operatorname{Var}(y_i) = \Pr(y_i)[1 - \Pr(y_i)]$, which, for the LPM, equals $X_i\beta(1 - X_i\beta)$. (Non-zero coefficients imply heteroskedasticity.)
...
As an initial simple example, let's use the `turnout` data set from the old {Zelig} package. {Zelig} has been replaced with {clarify}, but thankful jrnold saved Zelig's example data in the package [{ZeligData}](https://github.com/jrnold/ZeligData).
...
We can use least squares to fit a linear probability model to these data, treating the binary 0/1 `vote` variable as numeric.
...
To suitably link the bounded parameter $\pi_i$ with the unbounded linear predictor $X_i\beta$, we use the "inverse link function." The inverse link function wraps around the linear predictor and forces its values into the desired domain.[^03-logistic-regression-1]
...
For the Bernoulli distribution, we might use the inverse link function $g^{-1}(x) = \frac{e^x}{1 + e^x}$.[^03-logistic-regression-2] This is called the "inverse logit" and it has an "S"-shape.[^03-logistic-regression-3] Its job is to map $X\beta$ into $[0, 1]$.[^03-logistic-regression-4]
...
$$
y_i \sim \text{Bernoulli}(\pi_i)\text{, where } \pi_i = \operatorname{logit}^{-1}(X_i\beta).
$$
...
$$
f(y; \beta) = L(\beta) = \prod_{i = 1}^{N}\pi_i^{y_i} (1 - \pi_i)^{(1 - y_i)}\text{, where } \pi_i = \operatorname{logit}^{-1}(X_i\beta)
$$ Taking the log, we have

$$
\ell(\beta) = \sum_{i = 1}^{N} y_i \log \pi_i +  \sum_{i = 1}^{N}(1 - y_i) \log(1 - \pi_i)\text{, where } \pi_i = \operatorname{logit}^{-1}(X_i\beta)
$$
...
The tricky part about using `optim()` here is not the log-likelihood function, but setting up `X` and `y`.
...
\frac{\partial \pi_i}{\partial x_{i1}}
  &= \frac{\partial \pi_i}{\partial \eta_i}\cdot \frac{\partial \eta_i}{\partial x_{i1}}
   \;=\; \pi_i(1-\pi_i)\,\beta_1 = \left[ \operatorname{logit}^{-1}(X_i\beta) \right] \cdot \left[1 - \operatorname{logit}^{-1}(X_i\beta) \right] \cdot \beta_1
\end{aligned}
$$
...
# the anchor rows must be invisible through fill and colour, not alpha alone:
# the label box's fill and border ignore the alpha aesthetic, so alpha = 0
# anchors render as ~40 empty boxes across the panel
repel_df <- bind_rows(
  tibble(
    x = x_star,
    y = p(x_star, c(0,2)),
    ...
    alpha = 1, fill = "white", colour = "black"
  ),
  df |> group_by(z) |> slice(which(row_number() %% 3 == 1)) |> ungroup() |>
    transmute(x, y = p, label = '""', alpha = 0, fill = NA, colour = NA)
)
...
ggplot(df, aes(x, p)) +
  ...
  geom_label_repel(
    data = repel_df,
    aes(x, y, label = label, alpha = alpha, fill = fill, colour = colour),
...
Given that the coefficients are not directly interpretable, we can focus on other, simpler quantities of interest.

We have two core quantities of interest:
...
se_ev_hat <- sqrt(grad_ev %*% fit$var_hat %*% grad_ev)
```
...
for (i in 1:nrow(X_c)) {
  ...
  se_ev_hat[i] <- sqrt(grad_ev %*% fit$var_hat %*% grad_ev)
}
...
grad_fd <- grad(
  ...
  hi = X_hi,
  lo = X_lo)  
se_fd_hat <- sqrt(grad_fd %*% fit$var_hat %*% grad_fd)
...
We'll later see how to use {marginaleffects} to compute quantities of interest. But for now, realize that we can find the coefficient estimates with `coef()` and the estimated covariance matrix with `vcov()`. We can use these to manually obtain estimates of quantities of interest and their standard errors without {marginaleffects}. 
