...
If we create a column vector of coefficients $\beta = [\beta_0, \beta_1, ..., \beta_k]^\top$ and create a matrix $X$ by stacking a column of ones and the predictors side-by-side
...
Thus, a one-unit increase in $x_{ij}$ changes $\mu_i$ by $\beta_j$, holding the other predictor constant.
...
The squared and cubic terms are simply new columns in the design matrix. When fitting the model, these columns are treated just like any other predictor; they are just more columns in the design matrix.
...
We can include categorical variables with more than two categories by creating multiple indicator variables. Suppose that $x_{i1}$ and $x_{i2}$ are numeric variables and our categorical variable has three categories, labeled $A$, $B$, and $C$. We might define $x_{i3}=1$ if the observation is in category $B$ and $0$ otherwise, and $x_{i4}=1$ if the observation is in category $C$ and $0$ otherwise. Then the model would be
