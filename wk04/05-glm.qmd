...
1.  **Link function.** Monotone map $g(\mu_i) = \eta_i$ with $\mu_i = E(Y_i)$.
...
For the models we use, $a(\phi) = \phi$, so I write $\phi$ rather than $a(\phi)$ below.

If we choose $g(\mu)$ so that the linear predictor $\eta_i = X_i \beta$ equals the canonical parameter $\theta_i$, then we say that $g(\cdot)$ is the *canonical link.* Canonical links simplify the score equations and IRLS weights, but are not required in practice.


### Core examples

**Logistic regression.** $Y_i \sim \text{Bernoulli}(\pi_i)$ with canonical link $g(\pi_i)=\log\!\dfrac{\pi_i}{1-\pi_i}=X_i\beta$.

**Poisson regression (counts).** $Y_i \sim \text{Poisson}(\lambda_i)$ with canonical link $g(\lambda_i)=\log\lambda_i=X_i\beta$.
...
-   Weights: $w_i = \left(\dfrac{d\mu_i}{d\eta_i}\right)^2 \big/ V(\mu_i)$.\
-   Update: $\hat\beta \leftarrow (X^\top W X)^{-1} X^\top W z$. 

Repeat until convergence.

This algorithm is historically important, but happens "behind the scenes" in modern computation. It's more conceptually useful to imagine a generic gradient ascent algorithm applied directly to the multidimensional log-likelihood.

## Expected Fisher information

At the MLE, $\mathcal{I}(\beta) = \tfrac{1}{\phi} X^\top W X$. This looks very similar to the weighted cross-product matrix used in the IRLS updating step. There's a connection here. IRLS arises from applying Newton–Raphson to the gradient of the log-likelihood, and the Hessian of the log-likelihood (whose negative expectation is the Fisher information) uses the same $W$ matrix. 

As an interesting result, the weights $W$ indicate both the direction of the iterative updates and the curvature of the likelihood surface. When the algorithm has converged, $\phi\,(X^\top W X)^{-1}$ approximates the covariance matrix for $\hat\beta$.
