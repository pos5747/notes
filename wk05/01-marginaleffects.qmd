...
These coefficients are not particularly interpretable, so we might want to compute quantities of interest.^[In political science, @King2000 prompted the focus on more interpretable quantities and I often borrow their language.]

We have learned that we can use the invariance property and the delta method as a very general way to compute almost any quantity of interest. The {marginaleffects} package in R exploits this generality to compute many quantities of interest, especially those that involve $E(y)$--like the commonly used expected value and first difference. 
...
By default, `predictions()` generates an expected value for every row in the observed dataset. These expected values are stored in the `estimate` column of the output and it includes other commonly used values like the upper and lower bounds of the 95% confidence interval.
...
Reminder: first difference = $E(y \mid X_{hi}) - E(y \mid X_{lo})$, where $X_{hi}$ and $X_{lo}$ are carefully chosen covariate vectors that fix one covariate of interest at a high value and low value respectively.

`comparisons()` computes these first differences.
...
These first differences are stored in the `estimate` column of the output and it includes other commonly used values like the upper and lower bounds of the 95% confidence interval.
...
Thus, by default, `comparisons()` gives us a data frame with the observed data repeated several times for several different first differences. This is a reasonable default behavior, but we'll almost always want to compute something more specific (and avoid computing things we don't need).
...
Borrowing language from @King2000, I refer to {marginaleffects}'s "prediction" as an "expected value."
...
The "first difference" from @King2000 is an example of a comparison--a "difference" in the quote above.
...
# high = 1 SD above the mean; low = 1 SD below the mean
comparisons(fit, variables = list(age = "2sd")) |>
  glimpse()

# high = maximum; low = minimum
comparisons(fit, variables = list(age = "minmax")) |>
  glimpse()
```
...
```{r}
#  high = 90; low = 18; comparison = ratio
comparisons(fit, 
...
```{r}
#  high = 90; low = 18; comparison = custom (percent change)
comparisons(fit, 
            variables = list(age = c(18, 90)), 
...
For example, we can supply `newdata = datagrid(grid_type = "mean_or_mode")` to `predictions()`. This computes the prediction for a "typical case" that has numeric variables equal to their mean and factor variables equal to their mode.^[For integer variables like `age`, {marginaleffects} rounds the mean to the nearest integer (45.3 becomes 45).]

```{r}
predictions(fit, newdata = datagrid(grid_type = "mean_or_mode")) |>
  glimpse()
```
...
Because `predictions()` returns a data frame, we can plot the result. Here is the expected probability of voting across ages, with a 95% confidence interval.

```{r}
ev_age <- predictions(fit, newdata = datagrid(age = 18:90))

ggplot(ev_age, aes(x = age, y = estimate, ymin = conf.low, ymax = conf.high)) +
...
Notice that the last example gives `datagrid()` a function (`unique`) rather than values. `datagrid()` applies the function to the variable, so `race = unique` gives us every level of `race`. Other useful functions are `range` (the minimum and maximum) and `median`. If we name more than one variable, `datagrid()` creates every combination of their values. For example, `datagrid(age = range, race = unique)` has four rows.
...
`avg_predictions()` does the same for expected values.

```{r}
avg_predictions(fit)
```

The `by` argument averages within groups rather than across the whole grid. For example, we can average the first differences separately for white and non-white respondents.
...
When trying to find a representative summary of "the effect" from a non-linear model in which the effects vary, @hanmer2013 note that there are two approaches. 
...
In many cases the two approaches give similar estimates, but these are *different estimands* and sometimes the estimates meaningfully diverge. @beger2025 discuss potential differences.
