...
$$
\Pr(Y_i \leq j) = \operatorname{logit}^{-1}(\alpha_j - \eta_i),
\qquad j = 1, \ldots, J-1.
$$
...
$$
\begin{align}
\Pr(Y_i = 1) &= \operatorname{logit}^{-1}(\alpha_1 - \eta_i) \\
\Pr(Y_i = 2) &= \operatorname{logit}^{-1}(\alpha_2 - \eta_i) - \operatorname{logit}^{-1}(\alpha_{1} - \eta_i) \\
\Pr(Y_i = 3) &= \operatorname{logit}^{-1}(\alpha_3 - \eta_i) - \operatorname{logit}^{-1}(\alpha_{2} - \eta_i) \\
             &\vdots\\
\Pr(Y_i = j) &= \overbrace{\operatorname{logit}^{-1}(\alpha_j - \eta_i)}^{\Pr(Y_i \leq j)} - \overbrace{\operatorname{logit}^{-1}(\alpha_{j-1} - \eta_i)}^{^{\Pr(Y_i \leq j-1)}}, \qquad j = 2, \ldots, J-1 \\
             &\vdots\\
\Pr(Y_i = J) &= 1 - \operatorname{logit}^{-1}(\alpha_{J-1} - \eta_i).
\end{align}
$$
