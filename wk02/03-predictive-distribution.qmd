...
1. Simulate a new outcome variable using the estimated model parameters (i.e., $f(y; \hat{\theta})$). Perhaps simulate several for comparison.
...
Simply printing a few results, we can immediately see a problem: the simulated data look quite different from the observed data.
...
On the whole, we see here a close correspondence between the observed and simulated data. That suggests that our model is a good description of the data.

If we use the ECDFs rather than the histograms, we see a similarly well-fitting model.
```{r}
# plot ecdfs of real and fake data
ggplot(gg_data, aes(x = batting_average)) +
  stat_ecdf() +
  facet_wrap(vars(type)) +
...
If we look *really* hard, we can start to see some small differences between the observed data and the model, but the model is a really good one. With only two parameters, the beta distribution does an excellent job of recreating the distribution of the observed data.