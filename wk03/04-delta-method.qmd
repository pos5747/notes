...
From there, we can create Wald confidence intervals in the usual way. Recall that $\widehat{\operatorname{SE}}(\hat{\tau}) = \sqrt{\widehat{\operatorname{Var}}(\hat\tau)}$.

$$
\begin{align*}
 90\%~\text{C.I.}  &= \hat{\tau} \pm 1.64 \cdot \widehat{\operatorname{SE}}(\hat{\tau})\\
 95\%~\text{C.I.}  &= \hat{\tau} \pm 1.96 \cdot \widehat{\operatorname{SE}}(\hat{\tau})
\end{align*}
$$
...
Suppose a Bernoulli model of a binary outcome $y$. The ML estimate of $\pi$ is $\hat{\pi} = \operatorname{avg}(y)$. Using the Fisher information, the variance of $\hat{\pi}$ is $\widehat{\operatorname{Var}}(\hat{\pi}) = \dfrac{\hat{\pi}(1 - \hat{\pi})}{N}$.

Suppose we want to transform $\pi$ to the odds, where $\text{odds} = \tau(\pi) = \dfrac{\pi}{1 - \pi}$. We use the invariance property to obtain an ML estimate $\widehat{\text{odds}} = \dfrac{\hat\pi}{1 - \hat\pi}$.
...
$$
\widehat{\operatorname{SE}}(\widehat{\text{odds}}) \;\approx\; \sqrt{\dfrac{\hat{\pi}}{N(1 - \hat{\pi})^3}}.
$$
...
Suppose a Poisson model of a count variable $y$. The ML estimate is $\hat{\lambda} = \operatorname{avg}(y)$. We can use the information matrix to estimate the variance $\widehat{\operatorname{Var}}(\hat{\lambda}) = \dfrac{\hat{\lambda}}{N}$.
...
Simplifying, we have $\widehat{\operatorname{Var}}(\hat{\sigma}) \approx \dfrac{1}{4N}$ and $\widehat{\operatorname{SE}}(\hat{\sigma}) \;\approx\; \dfrac{1}{2\sqrt{N}}$.
...
$$
\nabla \tau(\hat\alpha, \hat\beta) =\begin{bmatrix}\dfrac{\hat\beta}{(\hat\alpha+\hat\beta)^2}\\-\dfrac{\hat\alpha}{(\hat\alpha+\hat\beta)^2}\end{bmatrix} = \begin{bmatrix}\dfrac{114.93}{(37.08+114.93)^2}\\[6pt]-\dfrac{37.08}{(37.08+114.93)^2}\end{bmatrix} = \begin{bmatrix} 0.004974 \\ -0.001605\end{bmatrix}
$$
...
To remind us what R is doing with `t(grad) %*% fit$var_hat %*% grad`, here is what the matrix multiplication looks like "by hand." I carry four significant figures throughout, because the two terms inside each entry nearly cancel---rounding earlier gives a visibly wrong answer.

$$
\begin{aligned}
\widehat{\operatorname{Var}}(\widehat{\mu})
&\approx
\overbrace{\begin{bmatrix} 0.004974 & -0.001605 \end{bmatrix}}^{\nabla\tau(\hat\alpha,\hat\beta)^{\!\top}}
\Biggl(
\underbrace{\begin{bmatrix} 5.964 & 18.407\\[2pt] 18.407 & 57.831 \end{bmatrix}}_{\widehat{\operatorname{Var}}(\hat\alpha,\hat\beta)}
\underbrace{\begin{bmatrix} 0.004974\\[2pt] -0.001605 \end{bmatrix}}_{\nabla\tau(\hat\alpha,\hat\beta)}
\Biggr)
&& \text{plug in values} \\[10pt]
&=
\overbrace{\begin{bmatrix} 0.004974 & -0.001605 \end{bmatrix}}^{\nabla\tau^{\!\top}}
\begin{bmatrix}
5.964(0.004974) + 18.407(-0.001605)\\[2pt]
18.407(0.004974) + 57.831(-0.001605)
\end{bmatrix}
...
&& \text{simplify} \\[10pt]
&=
0.004974\cdot 0.0001217 \;+\; (-0.001605)\cdot(-0.001261)
&& \text{multiply; (1 x 2) x (2 x 1)} \\[6pt]
&\approx
0.00000061 \;+\; 0.00000202
&& \text{simplify} \\[6pt]
&=
0.0000026
&& \text{simplify}
...
tbl <- data.frame(
  ...
  `How mean and SE are estimated` = c(
    "$\\hat\\mu = \\operatorname{avg}(y)$. $\\widehat{\\operatorname{SE}} = \\frac{\\operatorname{SD}(y)}{\\sqrt{N}}$.",
...
# compute the gradient of tau (a new name, so numDeriv's grad() stays usable)
grad_tau <- grad(func = tau_fn, x = fit$theta_hat)

# delta method
var_hat_mu <- grad_tau %*% fit$var_hat %*% grad_tau  # R transposes as needed
var_hat_mu
