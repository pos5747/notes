...
1.  **Input:** Observed data $y_1, y_2, \dots, y_N$, a parametric model $f(y; \theta)$, an estimator $\hat{\theta}$, and a quantity of interest $\hat{\tau} = \tau(\hat{\theta})$.[^02-parametric-bootstrap-1]
2.  Compute $\hat{\theta}$ from the observed data.
3.  For $b = 1, 2, \dots, B$:[^02-parametric-bootstrap-2]
    1.  Generate a bootstrap sample $y_1^{*(b)}, y_2^{*(b)}, \dots, y_N^{*(b)} \sim f(y; \hat{\theta})$. That is, simulate a new data set from the fitted parametric model.[^02-parametric-bootstrap-3]
    2.  Fit the model to the simulated data set $y_1^{*(b)}, y_2^{*(b)}, \dots, y_N^{*(b)}$ and compute $\hat{\theta}^{*(b)}$.
    3.  Compute $\hat{\tau}^{*(b)} = \tau \left( \hat{\theta}^{*(b)}\right)$, which might be $\hat{\theta}^{*(b)}$ itself if no transformation is needed.
...
[^02-parametric-bootstrap-1]: This works for *any* statistic that is a function of $y_1, \dots, y_N$, but we'll mainly use ML estimators and direct transformations of those.

[^02-parametric-bootstrap-2]: We typically use $B = 2,000$ as a good starting point.

[^02-parametric-bootstrap-3]: For example, if we use an exponential model and estimate the rate as $\hat{\lambda}$, then we would simulate each data set from an exponential distribution with parameter $\hat{\lambda}$. The first data set is $y_1^{*(1)}, \dots, y_N^{*(1)}$, the second data set is $y_1^{*(2)}, \dots, y_N^{*(2)}$, and so on. In R, that would be `rexp(N, rate = lambda_hat)`
...
We leave an evaluation of this confidence interval (i.e., Does it capture $\pi$ 90% of the time?) to later.
