...
As you can see, the ML estimates vary from sample to sample---different data sets produce different ML estimates.
...
&= \operatorname{Var}\left( \frac{1}{150} \sum_{i=1}^{150} y_i \right) 
&& \text{definition of } \hat{\pi} \\
&= \frac{1}{150^2} \sum_{i=1}^{150} \operatorname{Var}(y_i) 
&& \text{variance rule for sum of independent variables; } \operatorname{Var}(cX) = c^2 \operatorname{Var}(X) \\
&= \frac{1}{150^2} \cdot 150 \cdot \operatorname{Var}(y_i) 
&& \text{each } y_i \text{ has same variance because they are iid} \\
&= \frac{1}{150} \cdot \operatorname{Var}(y_i) 
...
$$
\begin{align*}
\operatorname{Var}[\operatorname{avg}(y)]
&= \operatorname{Var}\left( \frac{1}{N} \sum_{i=1}^N y_i \right)
&& \text{definition of } \operatorname{avg}(y) \\
...
Lastly, the standardized sample mean follows a $t$ distribution, so that $\sqrt{N} \cdot \left[ \frac{\operatorname{avg}(y) - \mu}{\operatorname{SD}(y)} \right]$ follows a *t* distribution with $N - 1$ degrees of freedom. This sampling distribution is the foundation for the one-sample $t$-test.
...
Above, we saw that for the toothpaste cap problem with 150 tosses and a 5% chance of a top, the fraction of tosses in the sample has an expected value of 5%. This means that the ML estimator is unbiased in that situation.
...
Suppose we sample from an exponential distribution with unknown rate $\lambda$. The ML estimator of the rate $\lambda$ is $\hat{\lambda}^{ML} = \frac{1}{\operatorname{avg}(y)}$.

To compute bias, we compute the expectation $E \left( \hat{\lambda}^{ML} \right) = E\left( \frac{1}{\operatorname{avg}(y)} \right)$.

There's a trick. Notice that $\frac{1}{x}$ is convex for $x > 0$.^[A function is **convex** if the function never curves above the line between any two of its points.] Jensen's Inequality states that if $g()$ is a convex function and $X$ is a random variable, then $g(E(X)) \leq E(g(X))$; moving an expectation inside a convex function decreases the value. Applying Jensen's inequality, we know that $E\left( \frac{1}{\operatorname{avg}(y)} \right) > \frac{1}{E[\operatorname{avg}(y)]}$.

But $\operatorname{avg}(y)$ is the sample mean of iid exponential random variables, so $E[\operatorname{avg}(y)] = \frac{1}{\lambda}$. It must be, then, that $E \left( \hat{\lambda}^{ML} \right) = E\left( \frac{1}{\operatorname{avg}(y)} \right) > \lambda$. Thus, $\hat{\lambda}^{ML}$ overestimates the true rate on average and is a biased estimator.
...
for (i in 1:n_mc_sims) {
  y <- rexp(sample_size, rate = lambda)
  lambda_hat[i] <- 1 / mean(y)  # ml estimate
}
...
For many problems, we know that we can create a 90% confidence interval for $\theta$ using $[\hat{\theta} - 1.64 \cdot \widehat{\operatorname{SE}}(\hat{\theta}), \hat{\theta} + 1.64 \cdot \widehat{\operatorname{SE}}(\hat{\theta})]$, where $\widehat{\operatorname{SE}}(\hat{\theta})$ is the *estimate* of the standard error of the sampling distribution.

Similarly, we can compute the *p*-value for the one-sided test of the null hypothesis that $\theta = 0$ against the alternative that $\theta > 0$ using $1 - \Phi\left( \frac{\hat{\theta}}{\widehat{\operatorname{SE}}(\hat{\theta})} \right)$, where $\Phi(\cdot)$ is the standard normal CDF.
...
Suppose $N$ samples $y_1, \dots, y_N$ from a Bernoulli distribution with parameter $\pi$. Let $\hat{\pi} = \operatorname{avg}(y)$. We can compute the SE of $\hat{\pi}$ analytically, but we work with the variance first, since that's easier.
...
The standard error is the square root of the variance, so that $\operatorname{SE}(\hat{\pi}) = \sqrt{ \frac{\pi(1 - \pi)}{N} }$.^[To obtain a commonly-used formula and preview a later result, we can plug in the estimator $\hat{\pi}$ into $\operatorname{SE}(\hat{\pi}) = \sqrt{ \frac{\pi(1 - \pi)}{N} }$ to obtain an *estimate* of the SE, so that $\widehat{\operatorname{SE}}(\hat{\pi}) = \sqrt{ \frac{\hat{\pi}(1 - \hat{\pi})}{N} }$.]

### Example: Exponential Model

Suppose we have $N$ samples $y_1, \dots, y_N$ from an exponential distribution and estimate the rate $\lambda$ with $\hat{\lambda}^{ML} = \frac{1}{\operatorname{avg}(y)}$. It isn't easy to work out the variance analytically, but a simulation works just fine for particular values of $N$ and $\lambda$.
