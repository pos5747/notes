...
We evaluate confidence intervals in terms of their **coverage**: a $C\%$ confidence interval should capture the parameter $C\%$ of the time under repeated sampling. That is, if we imagine repeating the study over and over (in the usual frequentist sense), then $C\%$ of the confidence intervals contain the true parameter.

Most of the interval estimators we use have asymptotic guarantees about coverage (i.e., the coverage approaches $C\%$ as the sample size grows large). However, we might want to use simulations to (1) confirm these results, (2) check the coverage for small samples, or (3) check the coverage when assumptions do not hold.
...
As an example, let's consider the usual 90% confidence interval for the mean: 90% CI = $[\operatorname{avg}(y) - 1.64 \times \widehat{\operatorname{SE}}, \operatorname{avg}(y) + 1.64 \times \widehat{\operatorname{SE}}]$, where $\widehat{\operatorname{SE}} = \frac{\operatorname{SD}(y)}{\sqrt{N}}$. We learned in an earlier class that for iid $y$, this interval should capture the population average in about 90% of repeated trials.
