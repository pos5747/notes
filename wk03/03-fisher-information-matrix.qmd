...
## $\ell(\theta)$: A new notation for the log-likelihood

Last week, we introduced the liklihood function $L(\theta)$ and the log-likelihood function $\log L(\theta)$. To emphasize and clarify, it made a lot of sense to denote the log-likelihood function as $\log L(\theta)$. 

However, we need something more compact this week, so we're going to use $\log L(\theta) = \ell(\theta)$. That is, whenever you see $\ell(\theta)$, that's just the same old log-likelihood function $\log L(\theta)$.

:::

$$
\begin{aligned}
\log L(\mu) = \ell(\mu) &= -\tfrac{N}{2}\log(2\pi) - \tfrac{1}{2}\sum_{i = 1}^N (y_i - \mu)^2\\
...
- As $N$ increases, $\dfrac{\partial^2 \ell(\mu)}{\partial \mu^2}$ becomes more negative.
- As the magnitude of $\dfrac{\partial^2 \ell(\mu)}{\partial \mu^2}$ increases, the curvature increases.
...
For a model $f(y; \theta)$ with log-likelihood $\ell(\theta) = \sum_{i=1}^N \log f(y_i; \theta)$, the **expected information** is defined as $\mathcal I(\theta) = -\mathbb E_\theta\!\left[\nabla^2 \ell(\theta)\right]$.
...
$$
\widehat{\operatorname{SE}}(\hat{\theta}) = \sqrt{\left[\left. - \dfrac{\partial^2 \ell(\theta)}{\partial \theta^2}\right| _{\theta = \hat{\theta}}\right] ^{-1}}
$$
...
\left[\left. - \dfrac{\partial^2 \ell(\mu)}{\partial \mu^2}\right| _{\mu = \hat{\mu}}\right] ^{-1} 
 = \dfrac{1}{N} 
...
$$
\begin{equation*}
\widehat{\operatorname{SE}}(\hat{\mu}) \approx \sqrt{\dfrac{1}{N}} 
\end{equation*}
$$

Does this answer make sense? What is the classic standard error of the mean when taking a random sample from a population? Hint: It's $\operatorname{SE}[\operatorname{avg}(y)] = \sigma/\sqrt{N}$. In this case, the "population SD" is $\sigma = 1$, as assumed by the *stylized* normal model.
...
It's a bit trickier to think about curvature in multiple dimensions. Instead of a second derivative like we have in a single dimension, we have a **Hessian** matrix in multiple dimensions.
...
alpha_grid <- alpha             # x-grid
beta_grid  <- beta              # y-grid
...
p4 <- p +
  ...
  annotate(
    ...
    family = "Source Sans 3", colour = "black",
  )
...
$$
\begin{equation*}
\widehat{\operatorname{Var}}(\hat{\theta})= \widehat{\operatorname{Cov}}(\hat{\theta}) \approx \left. \left[ 
\displaystyle \begin{matrix}
- \dfrac{\partial^2 \ell(\theta)}{\partial \theta_1^2} & - \dfrac{\partial^2 \ell(\theta)}{\partial \theta_1 \partial \theta_2}\\
...
\displaystyle \begin{matrix}
\color{red}{- \dfrac{\partial^2 \ell(\theta)}{\partial \theta_1^2}} & - \dfrac{\partial^2 \ell(\theta)}{\partial \theta_1 \partial \theta_2}\\
- \dfrac{\partial^2 \ell(\theta)}{\partial \theta_2 \partial \theta_1} & \color{red}{- \dfrac{\partial^2 \ell(\theta)}{\partial \theta_2^2}}\\
\end{matrix}\right]^{-1} \right|_{\theta = \hat{\theta}}
\end{equation*}
$$
...
\widehat{\operatorname{Var}}(\hat{\theta}) \approx \left. \left[ 
\displaystyle \begin{matrix}
- \dfrac{\partial^2 \ell(\theta)}{\partial \theta_1^2} & \color{blue}{- \dfrac{\partial^2 \ell(\theta)}{\partial \theta_1 \partial \theta_2}}\\
\color{blue}{- \dfrac{\partial^2 \ell(\theta)}{\partial \theta_2 \partial \theta_1}} & - \dfrac{\partial^2 \ell(\theta)}{\partial \theta_2^2}\\
\end{matrix}\right]^{-1} \right|_{\theta = \hat{\theta}}
\end{equation*}
$$
...
\widehat{\operatorname{Var}}(\hat{\theta}) &\approx \left. \left[ 
\displaystyle \begin{matrix}
- \dfrac{\partial^2 \ell(\theta)}{\partial \theta_1^2} & - \dfrac{\partial^2 \ell(\theta)}{\partial \theta_1 \partial \theta_2} & \ldots &- \dfrac{\partial^2 \ell(\theta)}{\partial \theta_1 \partial \theta_k}\\
- \dfrac{\partial^2 \ell(\theta)}{\partial \theta_2 \partial \theta_1} & - \dfrac{\partial^2 \ell(\theta)}{\partial \theta_2^2} & \ldots & - \dfrac{\partial^2 \ell(\theta)}{\partial \theta_2 \partial \theta_k}\\
\vdots & \vdots & \ddots & \vdots \\
- \dfrac{\partial^2 \ell(\theta)}{\partial \theta_k \partial \theta_1}     & - \dfrac{\partial^2 \ell(\theta)}{\partial \theta_k \partial \theta_2} & \ldots & - \dfrac{\partial^2 \ell(\theta)}{\partial \theta_k^2}\\
\end{matrix}\right]^{-1} \right|_{\theta = \hat{\theta}}\\
 & = \mathcal{I}_{\text{obs}}(\theta)^{-1}\big|_{\theta = \hat{\theta}}\\
...
Recall that $\widehat{\operatorname{SE}}(\hat{\theta}) = \sqrt{\left[\left. - \dfrac{\partial^2 \ell(\theta)}{\partial \theta^2}\right| _{\theta = \hat{\theta}}\right]^{-1}}$.

$$
\begin{align*}
 90\%~\text{C.I.}  &= \hat{\theta} \pm 1.64 \cdot \widehat{\operatorname{SE}}(\hat{\theta})\\
 95\%~\text{C.I.}  &= \hat{\theta} \pm 1.96 \cdot \widehat{\operatorname{SE}}(\hat{\theta})
\end{align*}
$$
...
In this example, we compute the standard error for the rate parameter in the exponential model. The exponential pdf is $f(y_i; \lambda) = \lambda \exp(-\lambda y_i)$ for $y_i \ge 0$ and the ML estimate is $\hat{\lambda} = \dfrac{1}{\operatorname{avg}(y)}$ (from earlier).
...
Invert to estimate the variance $\widehat{\operatorname{Var}}(\hat{\lambda}) \approx \dfrac{\hat{\lambda}^2}{N}$. Take the square root to get the standard error $\widehat{\operatorname{SE}}(\hat{\lambda}) \approx \dfrac{\hat{\lambda}}{\sqrt{N}}$.
...
At the maximum likelihood estimate, the first derivative is zero. That means we are sitting right on top of the hill. At that precise point, the slope is flat. To find the Hessian, we need to know how *quickly* the slope changes as we move away from the maximum. That "rate of change of the slope" is exactly the second derivative. In multiple dimensions, we call this matrix of second derivatives the **Hessian**.  
...
#| fig-cap: "A histogram of batting average in Lahman's data. Data from 2023; includes players with at least 100 at-bats."
#| column: margin
...
1. **Compute the log-likelihood.** `dbeta(..., log = TRUE)` returns $\log f(y_i; \alpha, \beta)$. Summing produces $\ell(\alpha,\beta) \;=\; \sum_{i=1}^N \log f(y_i; \alpha,\beta)$. We work with the log-likelihood because (1) logging turns numerically unstable products into stable sums and (2) the Hessian of the log-likelihood is useful for estimating the variance.  
